...
import { updateScene, deleteScene, type StudioActor } from '@/lib/studio-service';
  ...
  async (request: NextRequest, { params }: { params: Promise<Record<string, string>> }) => {
    ...
    return ApiResponse.json(
      await updateScene(
        memberId,
        id,
        { title: str(body.title), synopsis: str(body.synopsis), dialogue: str(body.dialogue) },
...
export const DELETE = withPermission(
  ...
  async (request: NextRequest, { params }: { params: Promise<Record<string, string>> }) => {
    ...
    return ApiResponse.json(await deleteScene(memberId, id, buildActor(session, request), { bypassOwnership: bypass }));
  },
);
