...
import { duplicateProject, type StudioActor } from '@/lib/studio-service';
...
// 複製專案為新副本（owner = 呼叫者）。需有建立權；讀取來源時，具 view_all 者可複製他人專案。
export const POST = withPermission(
  ...
  async (request: NextRequest, { params }: { params: Promise<Record<string, string>> }) => {
    ...
    return ApiResponse.json(await duplicateProject(memberId, id, buildActor(session, request), { bypassOwnership: bypass }));
  },
);
