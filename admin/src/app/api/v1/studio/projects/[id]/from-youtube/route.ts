...
import { adaptStoryboardFromSource } from '@/lib/studio/interview';
...
// 從 YouTube 影片「改編」成本專案的分鏡（相同節奏、內容原創）。
//   body: { url?, source?, sceneId?, count?, persist? }
//   - url：YouTube 連結（best-effort 抓字幕；抓不到會回可行動錯誤請改貼 source）
//   - source：直接貼上的字幕逐字稿／腳本（最可靠，優先使用）
//   - persist=false（預設）→ 只回 { shots }（modal 預覽審核）；persist=true → createShot 落庫回看板
export const POST = withPermission(
  ...
  async (request: NextRequest, { params }: { params: Promise<Record<string, string>> }) => {
    ...
    let body: { url?: unknown; source?: unknown; sceneId?: unknown; count?: unknown; persist?: unknown };
