...
3. 確認是否有關聯的計劃書（`docs/plans/`）或 PRD（`docs/requirements/`）
4. **PRD 前置檢查**（大型需求且需求模糊時）：若判斷為大型需求且使用者口述不完整、未表達豁免意圖（未出現「直接做 / 不用 PRD / 快速做一下」等關鍵字），應先建議走 PRD 流程，於 `docs/requirements/doing/` 建立 PRD 與使用者反覆討論至 ✅ 已確認後，才進入 Spec 階段；否則直接繼續 Phase 2
