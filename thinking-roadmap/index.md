...
2. **使用者量級**：個人使用，無高併發需求。
3. **目前團隊對於此技術的熟悉度**
4. **3 年成長性**：目前少量使用者夠用，未來若擴成多人協作，Next.js 全端 + PostgreSQL 仍可水平延伸擴展，複雜運算或排程服務，可以搭配Nest Js 或c# 程式語言來輔助短版。
...
事後回想，有幾件事如果在 **第 2 節「程式碼基礎建設」** 階段就一次到位，後面整個流程會更順、也少了補做的成本：

| 該做的事                              | 為什麼一開始做比較好                                                                                                               |
| ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| **Fixed package version**             | 鎖死 `package.json` 版本（移除 `^` / `~`），避免 AI 在不同 session 跑 `npm install` 時拉到新版套件，造成「明明沒改 code 卻壞掉」。 |
| **ESLint / Prettier 規則**            | 一開始就寫好 `.eslintrc` + `.prettierrc`，AI 產出的 code 才會風格一致；後補規則時要回頭修一堆既有檔案。                            |
| **VS Code Settings**                  | `.vscode/settings.json` + `extensions.json` 統一 format on save、推薦套件，避免不同環境格式化結果不同造成假 diff。                 |
| **Security Scan**（Snyk / npm audit） | 在骨架階段就接好掃描，能在依賴剛引入時就攔下漏洞；等到功能寫滿才補，要處理的洞會多很多。                                           |

**結論**：基礎建設不是只有「跑得起來」就好，**版本鎖定、Lint、IDE 設定、安全掃描** 這四件事屬於「一開始多花半天、後面省下很多天」的投資，下次新專案會直接放進骨架 checklist。
